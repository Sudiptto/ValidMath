...
function start2() {
    ...
    const second_num1 = Math.floor(Math.random() * 1);
...
function answer2() {
 ...
 function start3() {
     const first_num2 = Math.floor(Math.random() * 10) + 1;
     const second_num2 = Math.floor(Math.random() * 10) + 1;
...
function start12() {
    ...
    const c1 = Math.floor(Math.random() * 2)+ 1; // Second coefficient  (exponent !)
    const constantt = Math.floor(Math.random() * 2)+ 1; // Constant value 
    const constant = Math.floor(Math.random() * 2) + 1;
    const x = Math.floor(Math.random() * 2) + 1; // random exponent 
    const x1 = Math.floor(Math.random() * 1) + 1; // random exponent  value! 
    const x_value = Math.floor(Math.random() * 2) + 1;  // random x value 
    //Create Solution Using product rule:
    solutionn = ((c * x) * (x_value ** (x - 1))) * (c1 * (x_value ** x1) + constantt) - (((c1 * x1) * (x_value ** (x1 - 1))) * (c * (x_value ** x) + constant));
    // Display Question 
    document.getElementById("questions11").innerHTML = '(' + c + 'x' + "<sup>" + x + "</sup>" + ' + ' + constant + ')' + ' / (' + c1 + 'x' + "<sup>" + x1 + "</sup>" + ' + ' + constantt + '):  Solve for x = ' + x_value ; 
    document.querySelector('#disabled11').disabled = true;
    calculus3 += solutionn;
    
}
function answer12() {
    let y = String(userinput11.value);
    
    // Check if the string is valid or not
    if (isNum(y)) {
        //let user_value = parseInt(y);
        let user_value = parseInt(y);
        if(user_value == calculus3) {
            var entry = {
                point: '5'}
            fetch ('/point', {
                method : "POST",
